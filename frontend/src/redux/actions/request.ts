...
export const fetchCourseRequests = (courseId: number) => {
    return async (dispatch: Dispatch<RequestAction>) => {
        try {
            dispatch({type: RequestActionTypes.FETCH_COURSE_REQUESTS, loadable: true})
            ...
            dispatch({type: RequestActionTypes.FETCH_COURSE_REQUESTS_ERROR, payload: {code: error.response.status, message: `Failed to get requests for course with ID ${courseId}`}, throwable: true})
        }
    }
}
...
export const createCourseRequest = (code: string) => {
    return async (dispatch: Dispatch<RequestAction>) => {
        try {
            dispatch({type: RequestActionTypes.CREATE_COURSE_REQUEST, loadable: true})
            ...
            dispatch({type: RequestActionTypes.CREATE_COURSE_REQUEST_ERROR, payload: {code: error.response.status, message: 'Failed to send course request'}, throwable: true})
        }
    }
}
